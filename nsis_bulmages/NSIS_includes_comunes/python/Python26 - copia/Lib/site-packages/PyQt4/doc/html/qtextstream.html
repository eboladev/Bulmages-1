...
<p>Note that you cannot use <a href="qtextstream.html#atEnd">QTextStream.atEnd</a>(), which returns
true when you have reached the end of the data stream, with stdin.
The reason for this is that as long as stdin doesn't give any input
to the QTextStream, <tt>atEnd()</tt> will return true even if the
stdin is open and waiting for more characters.</p>
