...
/** @author Tomeu Borras
  ...
  * y realiza un informe de los errores encontrados para pasarselos al usuario. También proporciona el acceso de forma sencilla a los errores encontrados. 
  * El corrector sólo tiene una instancia en toda la ejecución del programa, es la clase empresa la que se encarga de construirlo y una vez construido permanece siempre en ejecución.
 */
...
Q_OBJECT
public:
	/// Base de datos con la que se está operando.
	...
	QString textBrowser;
	/// El corrector es una ventana del tipo dock, que se puede anexar a las esquinas del worspace de la aplicación. Este puntero apunta a la ventana contenedora del corrector.
	Q3DockWindow *dock;
public:
    correctorwidget( QWidget* parent = 0, const char* name = 0, Qt::WFlags fl = 0 );
    ~correctorwidget();
    /// El corrector, al contrario de todos los demás elementos tiene la inicialización de la base de datos y de la empresa realizada
    /// de forma diferente, debe usarse la función setEmpresa para inicializar el objeto. Esto es debido a que la construcción del corrector
    /// es anterior a la construcción de la clase empresa.
