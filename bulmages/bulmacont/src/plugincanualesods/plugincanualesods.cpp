...
Fixed pluginCAnualesODS::saldoCuenta(int cuenta) {
    ...
    cursor2 *cur;
    Fixed resultado;
    query.sprintf("SELECT (SUM(apunte.debe) - SUM(apunte.haber)) AS saldo FROM cuenta LEFT JOIN apunte ON apunte.idcuenta = cuenta.idcuenta WHERE cuenta.codigo like '%s%%' AND fecha >= '%s' AND fecha <= '%s'", QString::number(cuenta).toAscii().constData(), ejercicioActual_fechaInicio.toAscii().constData(), ejercicioActual_fechaFin.toAscii().constData());
...
Fixed pluginCAnualesODS::saldoCuentaAnt(int cuenta) {
    ...
    cursor2 *cur;
    Fixed resultado;
    query.sprintf("SELECT (SUM(apunte.debe) - SUM(apunte.haber)) AS saldo FROM cuenta LEFT JOIN apunte ON apunte.idcuenta = cuenta.idcuenta WHERE cuenta.codigo like '%s%%' AND fecha >= '%s' AND fecha <= '%s'", QString::number(cuenta).toAscii().constData(), ejercicioAnterior_fechaInicio.toAscii().constData(), ejercicioAnterior_fechaFin.toAscii().constData());
