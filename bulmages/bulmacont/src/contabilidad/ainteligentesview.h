 ...
 * \brief Edici� de Asientos Inteligentes
 * 
 * Clase que sirve para editar y crear asientos plantilla (inteligentes)
 * Los asientos intelignetes son un mecanismo mediante el cual podemos hacer la introducci� de asientos de una forma mucho m� r�ida que la convencional.
 */
class empresa;
...
class AInteligentesView : public QWidget, Ui_AInteligentesBase {
   Q_OBJECT
private:
/// La clase carga un cursor con todos los asientos inteligentes disponibles para hacer una navegaci� sobre �tos.
...
/// Para la generaci� de los centros de coste y canales en el listado hacen falta estos dos campos   
...
public: 
  ...
  ~AInteligentesView();
private:
/// Funci� que inicializa todos los controles del formulario a los valores adecuados
  void repinta();
  void return_asiento();
/// Funci� que se encarga de rellenar los canales disponibles y el array \ref m_cCanales  
  void cargacanales();
