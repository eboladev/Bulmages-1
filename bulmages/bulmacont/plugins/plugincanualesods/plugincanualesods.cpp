...
bool pluginCAnualesODS::Arboles()
{
    ...
    /// de una cuenta, vamos a crear una estructura tipo �rbol (usando la clase Arbol) donde,
    /// con un par consultas a BD, se averig�en todos los saldos y puedan estar disponibles en la RAM.
    ...
    while ( !ramas->eof() ) {
        if ( ramas->valor ( "nivel" ).toInt() == 2 ) { /// Cuenta ra�z.
    ...
    /// Inicializamos el �rbol desde sus raices (desde sus cuentas de nivel 2)
    /// con el resto de cuentas (las hojas del �rbol)
    ...
    /// establecer as� los valores de cada cuenta para el Ejercicio N.
    conexionbase->begin();
    ...
    /// establecer as� los valores de cada cuenta para el Ejercicio N-1.
    conexionbase->begin();
    ...
    /// De nuevo, para cada cuenta con sus saldos ya calculados hay que actualizar las hojas del �rbol.
    while ( !hojas->eof() ) {
