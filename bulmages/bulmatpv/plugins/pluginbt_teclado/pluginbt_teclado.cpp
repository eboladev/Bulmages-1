...
int entryPoint ( BtBulmaTPV *tpv )
{
    ...
    setlocale ( LC_ALL, "" );
    blBindTextDomain ( "pluginbt_teclado", g_confpr->valor ( CONF_DIR_TRADUCCION ).toAscii().constData() );
...
{
    blDebug ( "pluginbt_telcado::exitPoint", 0 );

    /// Vamos a probar con un docwindow.
    g_proc->kill();
    delete g_proc;
    delete g_docteclado;
    blDebug ( "END pluginbt_telcado::exitPoint", 0 );
    return 0;
}
