...
DECLARE
	as RECORD;
BEGIN
	SELECT INTO as * FROM configuracion WHERE nombre=''DBRev-SyncTPVBulmaCont'';
	IF FOUND THEN
		UPDATE CONFIGURACION SET valor=''0.11.1-001'' WHERE nombre=''DBRev-SyncTPVBulmaCont'';
	ELSE
		INSERT INTO configuracion (nombre, valor) VALUES (''DBRev-SyncTPVBulmaCont'', ''0.11.1-001'');
	END IF;
	RETURN 0;
END;
'   LANGUAGE plpgsql;
