...
BEGIN
        SELECT INTO rec * FROM pg_attribute WHERE attname = ''olvidadoalbaran'';
        IF NOT FOUND THEN
	    ALTER TABLE albaran ADD COLUMN olvidadoalbaran BOOLEAN DEFAULT FALSE;
...
DECLARE
	bs RECORD;
BEGIN
	SELECT INTO bs * FROM configuracion WHERE nombre=''DBRev-TicketsDevueltos'';
	IF FOUND THEN
		UPDATE CONFIGURACION SET valor=''0.11.1-0001'' WHERE nombre=''DBRev-TicketsDevueltos'';
	ELSE
		INSERT INTO configuracion (nombre, valor) VALUES (''DBRev-TicketsDevueltos'', ''0.11.1-0001'');
	END IF;
	RETURN 0;
END;
'   LANGUAGE plpgsql;
