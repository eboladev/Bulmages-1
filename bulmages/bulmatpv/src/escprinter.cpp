...
EscPrinter::EscPrinter(QString device)
{
	this->device = device;
}

void EscPrinter::print()
{
	QFile file ( device );
...
void EscPrinter::printAndClearBuffer()
{
	print();
	clearBuffer();
}

void EscPrinter::clearBuffer()
{
	buffer.clear();
}

void EscPrinter::initializePrinter()
{
	const char header[2] = {0x1B,0x40};
	buffer.append(QByteArray::fromRawData(header,2));
}

void EscPrinter::printText ( QString text )
{
	QTextCodec *codec = new QCP850EuroTextCodec ( 0 );
	if ( codec == NULL ){
		//ERROR
		return;
	}
...
void EscPrinter::printImage ( QString path )
{
...
//CHARACTER ESC CODES
void EscPrinter::setCharacterSpacing(unsigned char n)
{
	const char header[3]={0x1b,0x20,(char)n};
	buffer.append(QByteArray::fromRawData(header,3));
}

void EscPrinter::setCharacterCodeTable(characterCodeTable n)
{
	const char header[3] ={0x1B,0x74,(char)n};
	buffer.append(QByteArray::fromRawData(header,3));
}

void EscPrinter::setCharacterPrintMode(char mask)
{
	const char header[3] = {0x1B, 0x21, mask};
	buffer.append(QByteArray::fromRawData(header,3));
}

void EscPrinter::setUnderlineMode(bool mode)
{
	char n = mode?1:0;
...
void EscPrinter::setCharacterSize(char mask)
{
	const char header[3]={0x1D,0x21,mask};
	buffer.append(QByteArray::fromRawData(header,3));
}

void EscPrinter::setSmoothing(bool mode)
{
	char n = mode?1:0;
...
void EscPrinter::setDoubleStrike(bool mode)
{
	char n = mode?1:0;
...
void EscPrinter::turnUpsideDown(bool mode)
{
	char n = mode?1:0;
...
void EscPrinter::turn90CWRotation( bool onoff, bool extraSpacing)
{
	char n;
...
void EscPrinter::turnWhiteBlack( bool mode)
{
	char n = mode?1:0;
...
void EscPrinter::setColor(printColor color)
{
...
void EscPrinter::horizontalTab()
{
	buffer.append(0x09);
}

void EscPrinter::setHorizontalTabPos(int numtabs, char* tabspos)
{
	if(0>=numtabs>32 )
		return;
...
void EscPrinter::setLeftMargin(unsigned short margin)
{
	char nL = ( char ) ( margin );
...
void EscPrinter::setPrintingAreaWidth(unsigned short width) //width default = 512
{
	char nL = ( char ) ( width );
...
void EscPrinter::setJustification( justification j)
{
	const char header[3] = {0x1B,0x61,(char)j};
	buffer.append(QByteArray::fromRawData(header,3));
}

void EscPrinter::setHAbsolutePos(unsigned short pos)
{
	char nL = (char)pos;
	char nH = (char)(pos>>8);
...
void EscPrinter::setHRelativePos(unsigned short pos)
{
	char nL = (char)pos;
	char nH = (char)(pos>>8);
...
//Barcode commands
void EscPrinter::printBarCode(barcodeSystem m, int n, char* codeData)
{
	int error = 0;
	switch(m)
	{
...
void EscPrinter::setBarCodeHeight(int n)
{
	const char header[3] = {0x1D,0x68,(char)n};
	buffer.append(QByteArray::fromRawData(header,3));
}

void EscPrinter::setBarCodeWidth(int n)// 1<=n<=6 on tm-t88IV
{
	if(!(2<=n<=6))
		return; //ERROR
...
void EscPrinter::setBarcodeFormat(int width, int height, barCodeTextPos pos, printerFont font)// 1<=width<=6 on tm-t88IV
{
...
//Page mode
void EscPrinter::selectPageMode()
{
	const char header[2] = {0x1B,0x4C};
	buffer.append(QByteArray::fromRawData(header,2)); 
}

void EscPrinter::setPrintArea(unsigned short x, unsigned short y, unsigned short dx, unsigned short dy )
{
	char xL = (char)x;
	char xH = (char)(x>>8);
	char yL = (char)y;
...
void EscPrinter::setPrintDirection(printDirection dir)
{
	const char header[3] = {0x1B,0x54,(char)dir};
	buffer.append(QByteArray::fromRawData(header,3)); 
}

void EscPrinter::setVAbsolutePos(unsigned short pos)
{
	char nL = (char)pos;
	char nH = (char)(pos>>8);
...
void EscPrinter::setVRelativePos(unsigned short pos)
{
	char nL = (char)pos;
	char nH = (char)(pos>>8);
...
void EscPrinter::cancelPrintData()
{
	buffer.append(0x18);
}

//MACRO FUNCTION COMMANDS
void EscPrinter::startEndMacro()
{
	const char header[2] = {0x1D,0x3A};
	buffer.append(QByteArray::fromRawData(header,2));
}

void EscPrinter::executeMacro(int rep, int timePeriod, bool mode)
{
...
void EscPrinter::turnUnidirPrintMode(bool onoff) //No on TM-t88II TM-L60II
{
	const char header[3] = {0x1B,0x55,(char)onoff};
	buffer.append(QByteArray::fromRawData(header,3));
}

void EscPrinter::returnHome()//No on TM-t88II TM-L60II
{
	const char header[2] = {0x1B,0x3C};
	buffer.append(QByteArray::fromRawData(header,2));
}

void EscPrinter::cutPaper(bool partialcut) //ONLY partial cut on tm-t88II
{
	const char header[3] = {0x1D,0x56,(char)partialcut};
	buffer.append(QByteArray::fromRawData(header,3));
}

void EscPrinter::cutPaperAndFeed(bool partialcut, int feed)//ONLY partial cut on tm-t88II
{
...
//Print commands
void EscPrinter::printAndLineFeed()
{
	buffer.append(0x0A);
}

void EscPrinter::printAndReturnStandardMode()
{
	buffer.append(0x0C);
}

void EscPrinter::printAndCarriageReturn()
{
	buffer.append(0x0D);
}

void EscPrinter::printDataPageMode()
{
	const char header[2] = {0x1B,0x0C};
	buffer.append(QByteArray::fromRawData(header,2));
}

void EscPrinter::printAndFeedPaper(int feed)
{
	const char header[3] = {0x1B,0x4A,(char)feed};
	buffer.append(QByteArray::fromRawData(header,3));
}

void EscPrinter::printAndFeedLines(int lines)
{
	const char header[3] = {0x1B,0x64,(char)lines};
	buffer.append(QByteArray::fromRawData(header,3));
}

void EscPrinter::printAndReverseFeed(int feed) //Not compatible TM-t88II III 
{
...
void EscPrinter::selectDefaultLineSpacing()
{
	const char header[2] = {0x1B,0x32};
	buffer.append(QByteArray::fromRawData(header,2));
}

void EscPrinter::setLineSpacing(int lspacing)
{
...
//2D CODES
void EscPrinter::printQRCode(qrModel model, int dotSize, qrErrorCorrection error, unsigned short numData, char* data ) //1<=dotSize<=16
{
	numData+=3;
