...
BEGIN
	...
	SELECT INTO as * FROM pg_tables  WHERE tablename=''archivo'';
	IF NOT FOUND THEN
...
DECLARE
	as RECORD;
BEGIN
	SELECT INTO as * FROM configuracion WHERE nombre=''DBRev-Archivos'';
	IF FOUND THEN
		UPDATE CONFIGURACION SET valor=''0.11.1-0001'' WHERE nombre=''DBRev-Archivos'';
	ELSE
		INSERT INTO configuracion (nombre, valor) VALUES (''DBRev-Archivos'', ''0.11.1-0001'');
	END IF;
	RETURN 0;
END;
'   LANGUAGE plpgsql;
