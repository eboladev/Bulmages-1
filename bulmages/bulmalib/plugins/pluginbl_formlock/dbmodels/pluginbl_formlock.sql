--
-- Modificación de campos y funciones de la BD para la adaptacion para el plugin de Bloqueos
--
\echo "********* INICIADO FICHERO DE ESTRUCTURA DEL PLUGIN DE BLOQUEOS *********"
...
BEGIN
	SELECT INTO rs * FROM pg_tables  WHERE tablename=''bloqueo'';

	IF NOT FOUND THEN
	    CREATE TABLE bloqueo (
		idbloqueo SERIAL PRIMARY KEY,
		identificadorfichabloqueo varchar NOT NULL,
		fichabloqueo	varchar NOT NULL,
		usuariobloqueo  varchar NOT NULL,
...
BEGIN
	SELECT INTO rs * FROM configuracion WHERE nombre=''PluginBl_FormLock'';

	IF FOUND THEN
		UPDATE CONFIGURACION SET valor=''0.11.1-0001'' WHERE nombre=''PluginBl_FormLock'';
	ELSE
		INSERT INTO configuracion (nombre, valor) VALUES (''PluginBl_FormLock'', ''0.11.1-0001'');
	END IF;
