...
DECLARE
	as RECORD;
BEGIN
	SELECT INTO as * FROM configuracion WHERE nombre=''DBRev-Bloqueo'';
	IF FOUND THEN
		UPDATE CONFIGURACION SET valor=''0.11.1-0001'' WHERE nombre=''DBRev-Bloqueo'';
	ELSE
		INSERT INTO configuracion (nombre, valor) VALUES (''DBRev-Bloqueo'', ''0.11.1-0001'');
	END IF;
	RETURN 0;
END;
'   LANGUAGE plpgsql;
