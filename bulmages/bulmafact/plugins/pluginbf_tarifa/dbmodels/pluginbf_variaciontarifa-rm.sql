...
\echo "Eliminamos la tabla variaciontarifa."
...
DECLARE
	as RECORD;
BEGIN
	SELECT INTO as * FROM configuracion WHERE nombre=''PluginBf_VariacionTarifa'';
	IF FOUND THEN
		DELETE FROM CONFIGURACION WHERE nombre=''PluginBf_VariacionTarifa'';
	END IF;
	RETURN 0;
END;
'   LANGUAGE plpgsql;
