...
\return
**/
BcCuentaListView::BcCuentaListView ( BfCompany *comp, QWidget *parent, Qt::WindowFlags flag, edmode editmodo )
        : BlFormList ( comp, parent, flag, editmodo )
{
    ...
    mui_nivel->clear();
    for ( int i = g_confpr->value(CONF_CONT_NUMDIGITOSEMPRESA).toInt(); i >= 2; i-- ) {
...
void BcCuentaListView::editar ( int row )
{
    ...
    if ( editMode() ) {
        /*
                ArticuloView * art = ( ( BfCompany * ) BlMainCompany() ) ->newArticuloView();
...
void BcCuentaListView::crear()
{
    /*
        BL_FUNC_DEBUG
        ( ( BfCompany * ) BlMainCompany() ) ->s_newArticulo();
