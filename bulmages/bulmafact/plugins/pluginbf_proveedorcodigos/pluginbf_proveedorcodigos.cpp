...
#include "pluginbf_proveedorcodigos.h"
#include "bfcompany.h"
...
int entryPoint ( BfBulmaFact *bges )
{
    ...
    bges->company()->dbPatchVersionCheck("PluginBf_CodigosProveedor", "0.13.2");

    /// Inicializa el sistema de traducciones 'gettext'.
    setlocale ( LC_ALL, "" );
    blBindTextDomain ( "pluginbf_proveedorcodigos", g_confpr->value( CONF_DIR_TRADUCCION ).toLatin1().constData() );
...
int ArticuloView_ArticuloView ( ArticuloView *art )
{
    ...
    l->setObjectName ( QString::fromUtf8 ( "lcodprov" ) );
    l->setMainCompany ( art->mainCompany() );
    l->setDbTableName ( "codprov" );
    l->setDbFieldId ( "idcodprov" );
    l->addSubFormHeader ( "cifproveedor", BlDbField::DbVarChar, BlDbField::DbNoSave, BlSubFormHeader::DbNone , _ ( "CIF Proveedor" ) );
    l->addSubFormHeader ( "nomproveedor", BlDbField::DbInt, BlDbField::DbNoSave, BlSubFormHeader::DbNone | BlSubFormHeader::DbNoWrite , _ ( "Proveedor" ) );
    l->addSubFormHeader ( "cadcodprov", BlDbField::DbVarChar, BlDbField::DbNotNull, BlSubFormHeader::DbNone , _ ( "Codigo Proveedor" ) );
    ...
    l->addSubFormHeader ( "idcodprov", BlDbField::DbInt, BlDbField::DbPrimaryKey, BlSubFormHeader::DbHideView | BlSubFormHeader::DbNoWrite, _ ( "Id validacion" ) );
    
    l->addSubFormHeader ( "idproveedor", BlDbField::DbInt, BlDbField::DbNotNull, BlSubFormHeader::DbHideView | BlSubFormHeader::DbNoWrite , _ ( "Id proveedor" ) );
    ...
    art->mui_tab->addTab ( l, "Codigos Proveedor" );
...
{
    BL_FUNC_DEBUG
    BfSubForm *l = art->findChild<BfSubForm *> ( "lcodprov" );
    if ( l ) {
        l->load ( "SELECT * FROM codprov LEFT JOIN proveedor ON codprov.idproveedor = proveedor.idproveedor WHERE idarticulo = " + art->dbValue ( "idarticulo" ) );
    } // end if
