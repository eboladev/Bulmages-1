...
MyPlugAlm::MyPlugAlm()
{
    _depura ( "MyPlugAlm::MyPlugAlm", 0 );
    _depura ( "END MyPlugAlm::MyPlugAlm", 0 );
}
...
\param bges
**/
void MyPlugAlm::inicializa ( Bulmafact *bges )
{
    _depura ( "MyPlugAlm::inicializa", 0 );

    if ( bges->getcompany()->has_table_privilege ( "almacen", "SELECT" ) ) {
...
int entryPoint ( Bulmafact *bges )
{
    ...
    setlocale(LC_ALL, "");
    bindtextdomain ("pluginbf_Almacen", confpr->valor(CONF_DIR_TRADUCCION).toAscii().constData());
