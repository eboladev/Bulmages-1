...
BEGIN
	...
	IF FOUND THEN
		ALTER TABLE articulo DROP COLUMN ivaincarticulo;
	END IF;

	SELECT INTO as * FROM pg_attribute  WHERE attname=''pvpivainclalbaran'';
	IF FOUND THEN
		ALTER TABLE lalbaran DROP COLUMN pvpivainclalbaran;
		ALTER TABLE lfactura DROP COLUMN pvpivainclfactura;
		ALTER TABLE articulo DROP COLUMN pvpivaincarticulo;
	END IF;
...
DECLARE
	as RECORD;
BEGIN
	SELECT INTO as * FROM configuracion WHERE nombre=''PluginBf_IVAIncluido'';
	IF FOUND THEN
		DELETE FROM CONFIGURACION WHERE nombre=''PluginBf_IVAIncluido'';
	END IF;
	RETURN 0;
END;
'   LANGUAGE plpgsql;
