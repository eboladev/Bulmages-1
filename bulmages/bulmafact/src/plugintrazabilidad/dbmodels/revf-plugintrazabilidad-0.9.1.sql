...
DECLARE
	as RECORD;
BEGIN
	SELECT INTO as * FROM configuracion WHERE nombre=''DBRev-Trazabilidad'';
	IF FOUND THEN
		UPDATE CONFIGURACION SET valor=''0.9.1'' WHERE nombre=''DBRev-Trazabilidad'';
	ELSE
		INSERT INTO configuracion (nombre, valor) VALUES (''DBRev-Trazabilidad'', ''0.9.1'');
	END IF;
	RETURN 0;
END;
'   LANGUAGE plpgsql;
