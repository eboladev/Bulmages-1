...
void ClientInvoicesList::s_doubleclicked(int a, int , int , const QPoint &) {
    ...
    if (m_modo ==0 && m_idcliinvoice != "") {
        fprintf(stderr,"Cargamos la pantalla de edici� AlbaranClienteView\n");
        AlbaranClienteView *cInvoice = new AlbaranClienteView(companyact,companyact->m_pWorkspace,theApp->translate("Edicion de Albar� de Cliente", "company"));
...
void ClientInvoicesList::s_newClientInvoice() {
    ...
    AlbaranClienteView *cInvoice = new AlbaranClienteView(companyact,companyact->m_pWorkspace,theApp->translate("Edicion de Albaran� de Cliente", "company"));
    cInvoice->show();
}// end boton_crear
