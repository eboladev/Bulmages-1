...
void ListLinAlbaranClienteView::cargar(QString idalbaran) {
        ...
        cursor2 * cur= companyact()->cargacursor("SELECT * FROM lalbaran LEFT JOIN articulo ON lalbaranp.idarticulo = articulo.idarticulo WHERE idalbaranp="+mdb_idalbaran);
        SubForm2::cargar(cur);
        delete cur;
}
...
void ListLinAlbaranClienteView::valueBudgetLineChanged(int row, int col) {
    ...
    if (linea != NULL) {
        ...
        case COL_DESCONTLALBARAN: {
                Fixed discountLine(text(row, COL_DESCONTLALBARAN).replace(",",".").toAscii());
                linea->setdescontlalbaran(discountLine.toQString());
                break;
            }
        case COL_CODARTICULO:
            manageArticle(row);
        ...
        case COL_DESCLALBARAN:
            ...
            break;
        case COL_CANTLALBARAN: {
                Fixed cantLine(text(row, COL_CANTLALBARAN).replace(",",".").toAscii());
                linea->setcantlalbaran(cantLine.toQString());
                break;
            }
        case COL_PVPLALBARAN:  {
                Fixed pvpLine(text(row, COL_PVPLALBARAN).replace(",",".").toAscii());
                linea->setpvplalbaran(pvpLine.toQString());
                break;
            }
        }// end switch
...
QString ListLinAlbaranClienteView::searchArticle() {
    _depura("ListLinAlbaranClienteView::searchArticle\n",0);
